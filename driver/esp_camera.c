...
esp_err_t esp_camera_init(const camera_config_t *config)
{
    ...
    int ret=s_state->sensor.set_pixformat(&s_state->sensor, pix_format);
    ESP_LOGD(TAG, "Setting pix format  %d,result is %02x",pix_format,ret);
#if CONFIG_CAMERA_CONVERTER_ENABLED
    if(config->conv_mode) {
    ...
    ESP_LOGI(TAG, "0x11:%02X\t0x12:%02X\t0x20:%02X\t0x4A:%02X\t0x0C:%02X\t0x3A:%02X",SCCB_Read(s_state->sensor.slv_addr, 0x11),
    SCCB_Read(s_state->sensor.slv_addr, 0x12),SCCB_Read(s_state->sensor.slv_addr, 0x20),SCCB_Read(s_state->sensor.slv_addr, 0x4A),
    SCCB_Read(s_state->sensor.slv_addr, 0x0C),SCCB_Read(s_state->sensor.slv_addr, 0x3A));
